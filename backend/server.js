...
// It automatically looks for the GEMINI_API_KEY in the .env file
...
const corsOptions = {
    origin: function (origin, callback) {
        ...
        const allowedOrigins = [
            'http://localhost:3000',
            'http://127.0.0.1:3000',
            process.env.FRONTEND_URL, // Add your Render frontend URL in environment variables
        ...
        if (allowedOrigins.indexOf(origin) !== -1 || process.env.NODE_ENV === 'development') {
            ...
            callback(null, true); // Allow all origins in production (change to false for strict security)
        }
    },
...
app.post('/chat', async (req, res) => {
    try {
        ...
        // We use the last message from the user as the current message
        const currentMessage = { role: 'user', parts: [{ text: message }] };
        const historyWithCurrent = [...chatHistory, currentMessage];


        // Create a new chat session to maintain conversation history
        const chat = ai.chats.create({
            model: MODEL,
            history: historyWithCurrent.slice(0, -1), // Everything except the current user message
        });
